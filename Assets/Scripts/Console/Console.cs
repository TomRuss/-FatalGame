...
namespace DevConsole {
    ...
    public class Console : MonoBehaviour, ISerializationCallbackReceiver {
        ...
        public float helpWindowWidth = 200;                 //The width of the autocompletion window
        ...
        void OnEnable() {
            ...
            if(extraSettings.defaultCommands)
                AddCommands(
                    new Command("c_clear", Clear, "Clears the console"),
                    new Command("c_commands", HelpCommand, "Shows a list of all Commands available"),
                    new Command<string>("c_change_key", ChangeKey, ChangeKeyHelp),
                    new Command<int>("c_set_font_size", SetFontSize, "Sets the font size of the Console")
                );
        }
        ...
        void OnGUI() {
            ...
            if(!closed) {
                ...
                GUI.enabled = !opening;
                inputText = GUI.TextField(new Rect(0, currentConsoleHeight + 0, Screen.width, 35), inputText);
                GUI.enabled = true;
        ...
        void SetFontSize(int size) {
            if(size > 5 && size < 25) {
                fontSize = size;
                Log("Change successful", Color.green);
            } else LogError("The entered value needs to be between 5 and 25");
        }
        #endregion
    }
