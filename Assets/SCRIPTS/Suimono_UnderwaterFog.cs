...
namespace Suimono.Core
{
	...
	public class Suimono_UnderwaterFog : MonoBehaviour {
		...
		void LateUpdate () {
			...
			trans1Time += (Time.deltaTime*0.7f*wipeTime);
			trans2Time += (Time.deltaTime*0.1f*dropsTime);
		}
		...
		void OnRenderImage (RenderTexture source, RenderTexture destination){
			...
			if (fogMaterial != null){
				...
				if (moduleObject != null){
					hFac = Mathf.Clamp((11.5f) - moduleObject.setTrack.transform.localPosition.y, 0.0f,500.0f);
					heightDepth = hFac;
